...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Delete_participant` (IN `User_id` INT(11), IN `Chat_id` INT(11))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Insert_participant` (IN `User_id` INT(11), IN `Chat_id` INT(11))  BEGIN
INSERT INTO participate(Id_user,Id_chat)
VALUES(User_id,Chat_id);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Message_not_received` (IN `Chat_id` INT(11), IN `User_id` INT(11))  SELECT *
FROM message m, participate p
WHERE m.Id_chat = Chat_id
AND p.Id_user = User_id
AND p.Last_message_id < m.Message_number$$
